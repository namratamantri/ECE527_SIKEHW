...
#pragma HLS unroll
			digit_d temp = (ai[j]*bi[i]);
			partial_products[i][j] = (digit_d)temp << (P*j);
...
#pragma HLS unroll
	...
	digit_d mult=0;
	mult = sum[0] + sum[1] + sum[2] + sum[3] + sum[4] + sum[5] + sum[6] + sum[7];// + sum[7] + sum[8] + sum[9] + sum[10] + sum[11] + sum[12] + sum[13] ;
//	for(int i =0;i<N;i++)
//		mult += sum[i];
	*c = mult;
}
